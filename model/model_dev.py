...
def run(params,
    ...
    def boundary_fluxes(current, sea_level, dt, dx, p):
        ...
        # Topography (Eqn. 1)
        L1_bflux = 1 * ((K * (h_half - R(h_half)) / dx**2) * dt)  # for flux make positive; seems to check out (*1)
        R1_bflux = -1 * ((K * (L(h_half) - h_half) / dx**2) * dt) # should be negative; opposite, as math has been reversed (*-1)
    ...
    # I actually think this already takes care of the "new eroded surface" with the < d[::2] step!
    ...
    def update_sto(sto, d, i, sea_level, decay_fun=None,
        ...
        if mode == 'molar':
            ...
            if molorg_prev is not None:
                ...
                d13c_org = d13c_sw + ep # -25‰;  photosynthetic fractionation; can use a Rayleigh relationship here if I want later, d13c_org = d13c_org_i - e * ln(f)
                ...
                # mol balance (old version without accounting for mass of existing carbonate)
                # d13c_pf_raw = mol_bal(n_sw,d13c_sw,n_org,d13c_org)                
                # d13c_pf = rem_ol(d13c_pf_raw) # remove outliers
            ...
            # old method is complex and introduces nans due to masking with subaerial and erosion i think. use the one from grid which is calculated properly and without error
