...
def calc_mol(carb_frac=None,toc_percent=None, topo=None, mode='organic sediment', params=None,
    ...
    if mode == 'organic sediment':
        ...
        m_sed = (v_cm3) * rho_sed # as organics are in the porespaces, add phi
...
#     # changed: remove the organic share of what precipitated from the organic pool
#     # the precipitate comes from a mixture; only the organic fraction should be subtracted
#     frac_org = jnp.where(n_pf > 0, n_org / (n_pf + pad), 0.0)     # share of organics in the pre-precip porefluid
#     n_add_org = frac_org * n_add  
    
#     # organic moles that actually precipitated this step
#     n_org_pool_new = n_org #jnp.clip(n_org - n_add_org, 0.0, None)       # updated organic pool (non-negative)
